(() => {
 const SUPABASE_URL = "https://cfqvfiquhtzzzfuubltf.supabase.co";
  ...
  function atualizarDataCabecalho() {
    const elemento = document.getElementById("data-cabecalho");
    if (!elemento) return;
  ...
  function rolarContainer(containerId, seletorCard, direcao) {
    ...
    const distancia = card
      ? card.offsetWidth + 20
      : Math.min(container.clientWidth * 0.85, 800);
  ...
  window.rolarRecomendados = function rolarRecomendados(direcao) {
    rolarContainer("cardsRecomendados", ".RECO1", direcao);
  };

  window.rolarPopulares = function rolarPopulares(direcao) {
    rolarContainer("cardsPopulares", ".POPU1", direcao);
  };

  window.rolarMaisVisitados = function rolarMaisVisitados(direcao) {
    rolarContainer("cardsMaisVisitados", ".MAVI1", direcao);
  };
  ...
  function iniciarBusca() {
    const input = document.querySelector(".buscBarb");
    const botao = document.querySelector(".btnPesquisa");

    if (!input || input.dataset.buscaConfigurada === "true") {
      return;
    }

    input.dataset.buscaConfigurada = "true";
    ...
    function irParaBusca() {
      ...
      const url = new URL("./Page/Buscar/buscar.html", window.location.href);
    ...
    botao?.addEventListener("click", irParaBusca);
  }
  ...
  function fecharModais() {
    document.querySelectorAll(".modal-autenticacao").forEach((modal) => {
      modal.hidden = true;
    });
  }
  ...
  function abrirModal(seletor) {
    fecharModais();
    mostrar(document.querySelector(seletor));
  }
  ...
  function fecharDropdown() {
    ...
    const botao = document.getElementById("perfil-logado");
  ...
  function preencherAvatar(elemento, perfil) {
    if (!elemento) return;
    ...
    if (perfil.foto_perfil) {
      ...
      imagem.addEventListener(
        ...
        () => {
          elemento.innerHTML = "";
          elemento.textContent = obterInicial(perfil.nome);
        },
        { once: true }
      );
  ...
  async function buscarPerfil(user) {
    ...
    try {
      const { data, error } = await supabase
        .from("usuarios")
        .select("nome, email, foto_perfil, role")
        .eq("id", user.id)
        .maybeSingle();
      ...
      return {
        nome: data?.nome || user.user_metadata?.nome || user.email?.split("@")[0] || "Usuário",
  ...
  function mostrarDeslogado() {
    ...
    const agendamentos = document.getElementById("agendamentos-link");
    const saudacao = document.querySelector(".h2Cabecalho");
    ...
    // Oculta o card no topo se não houver login
    carregarAgendamentoTopo(null);

    if (saudacao) {
      saudacao.textContent = "Olá, faça seu login!";
    }

    if (!container) return;

    container.innerHTML = `
      <button type="button" id="perfil">
        Entrar
      </button>
    `;

    document.getElementById("perfil")?.addEventListener("click", () => abrirModal(".login"));
  }
  ...
  function mostrarLogado(perfil) {
    ...
    if (saudacao) {
      saudacao.textContent = `Olá, ${perfil.nome}!`;
    }

    container.innerHTML = `
      <button type="button" id="perfil-logado" aria-haspopup="true" aria-expanded="false">
        <span class="avatar-header"></span>
        <span class="nome-header"></span>
        <span class="seta-header" aria-hidden="true">▼</span>
      </button>
    `;

    const botao = document.getElementById("perfil-logado");
    const avatar = botao?.querySelector(".avatar-header");
    const nome = botao?.querySelector(".nome-header");

    if (nome) nome.textContent = perfil.nome;
    if (menuNome) menuNome.textContent = perfil.nome;
    if (menuEmail) menuEmail.textContent = perfil.email;

    preencherAvatar(avatar, perfil);
    preencherAvatar(menuAvatar, perfil);
  ...
  async function verificarLogin() {
    ...
    try {
      ...
      // Carrega dinamicamente o card de agendamento no topo
      await carregarAgendamentoTopo(user.id);
    } catch (error) {
  ...
  async function entrar() {
    ...
    const botao = document.getElementById("confirmar-login");
  ...
  async function cadastrar() {
    ...
    const botao = document.getElementById("confirmar-cadastro");
    ...
    try {
      ...
      if (!data.session) {
        alert("Conta criada! Confirme seu e-mail para entrar.");
        return;
      }
  ...
  async function reenviarConfirmacao() {
    const supabase = obterSupabase();
    const email = document.querySelector(".Email1")?.value.trim().toLowerCase();
    ...
    try {
      ...
      alert(`Não foi possível reenviar: ${error.message}`);
    }
  }
  ...
  window.btnReserva = async function btnReserva(barbeariaId) {
    const supabase = obterSupabase();
    ...
    if (!supabase) {
      alert("Não foi possível verificar sua conta.");
      return;
    }
    ...
    try {
      ...
      window.location.href = `./Page/Barbearia/barbearia.html?id=${encodeURIComponent(
        barbeariaId
      )}`;
    } catch (error) {
  ...
  function registrarEventos() {
    document.getElementById("perfil")?.addEventListener("click", () => abrirModal(".login"));
    document.getElementById("abrir-login")?.addEventListener("click", () => abrirModal(".Entrar"));
    document.getElementById("abrir-cadastro")?.addEventListener("click", () => abrirModal(".Cadastrar"));

    document.getElementById("confirmar-login")?.addEventListener("click", entrar);
    document.getElementById("confirmar-cadastro")?.addEventListener("click", cadastrar);
    document.getElementById("reenviar-confirmacao")?.addEventListener("click", reenviarConfirmacao);
    ...
    document.querySelectorAll(".voltar-modal").forEach((botao) => {
    ...
    document.addEventListener("click", (evento) => {
      const menu = document.getElementById("menu-usuario-dropdown");
      const perfil = document.getElementById("perfil-logado");

      if (menu?.contains(evento.target) || perfil?.contains(evento.target)) {
        return;
      }
    ...
    document.addEventListener("keydown", (evento) => {
      ...
      if (evento.key === "Enter" && document.activeElement?.classList.contains("Senha1")) {
        entrar();
      }

      if (evento.key === "Enter" && document.activeElement?.classList.contains("SenhaCadastro")) {
        cadastrar();
      }
    });
  }
  ...
  async function iniciar() {
    ...
    if (supabase && !inscricaoAuth) {
      const resposta = supabase.auth.onAuthStateChange(() => {
        setTimeout(() => {
          verificarLogin();
        }, 0);
      });

      inscricaoAuth = resposta.data.subscription;
    }
  }
